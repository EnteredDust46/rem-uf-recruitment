...
const BUILD_STAMP = 'rd2-grader-tabs-20260915';
...
function r2WeightedDimKeys() {
  return r2CaseDims().map(function (d) { return d.key; });
}
...
function bindR2BandOpts(root, a) {
  root.querySelectorAll('.band-opt').forEach(function (el) {
    ...
    el.addEventListener('click', function () {
      ...
      rec.scores[key] = cur === val ? undefined : val;
      if (isR2MathKey(key)) {
        rec.scores.quant_reasoning = rec.scores[key];
        rec.scores.math = rec.scores[key];
      }
      saveR2GraderNoteFields('round2', a.id, rec);
...
function r2DimCardHtml(d, g, applicantId, graderKey) {
  ...
  return `
      <div class="dim-card">
        <div class="dim-head"><h4>${esc(d.label)}</h4>${r2ScorePill(score)}</div>
        <div class="dim-body">
          ${r2DimGuideHtml(guideKey)}
          <div class="band-row" style="grid-template-columns: repeat(4,1fr);">
            ${d.levels.map((txt, i) => { const val = 4 - i; return `<div class="band-opt ${score === val ? 'sel' : ''}" data-key="${d.key}" data-val="${val}"><span class="sc">${R.levelLabels[i]}</span>${esc(txt)}</div>`; }).join('')}
          </div>
